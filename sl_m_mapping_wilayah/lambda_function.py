...
def master_table_name():
    return 'sl_05_m_mapping_wilayah'
...
def functionGet(con, event):
    ...
    FROM
        `sl_05_m_mapping_wilayah` map
        LEFT JOIN `sl_02_m_wilayah` wil ON ( wil.wilayah_code = map.wilayah_code ) 
        LEFT JOIN `sl_04_m_kelurahan` kel ON ( kel.kelurahan_code = map.kelurahan_code ) 
    WHERE
        map.delete_mark = 0 
        AND map.active = 1
    """
    ...
    for row in myresult:
        data = {
            ...
            "wilayah_code": row[1],
            "wilayah_name": row[2],
            "kelurahan_code": row[3],
            "kelurahan_name": row[4],
        }
        allData.append(data)
...
def functionPost(con, event):
    ...
    sqlInsert = """
    INSERT INTO `sl_05_m_mapping_wilayah` 
        (wilayah_code, kelurahan_code, active, create_by, create_date, delete_mark ) 
    VALUES 
        (%s, %s, %s, %s, %s, 0)
    """
    cur.execute(sqlInsert, (req['wilayah_code'], req['kelurahan_code'], req['active'], 
...
def functionPut(con, event):
    ...
    id = params['id']
    sqlUpdate = """
    UPDATE `sl_05_m_mapping_wilayah` 
    SET 
        `wilayah_code` = %s,
        `kelurahan_code` = %s,
        `update_by` = %s, 
    ...
    WHERE 
       `id` = %s
    """
    cur.execute(sqlUpdate, (req['wilayah_code'], req['kelurahan_code'], 
