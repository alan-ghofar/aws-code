...
tableName = 'in_01_t_adjustment_out'
...
def functionPost(con, event):
    ...
    try:
        ...
        sqlInsert = """
        call  `in_t_adjustments`
        ( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """
        cur.execute(sqlInsert, (idMenu, inKodePerusahaan, typeDocument, 2, str(req['adj_type']), str(req['kode_lokasi']), 
        req['trans_date'], str(req['memo']), str(req['status_code']), str(vToken['id_user']), stritm, trans_no))
...
def functionGet(con, event):
    ...
    FROM
        in_01_t_adjustment AS h
        LEFT JOIN in_02_t_adjustment_detail AS l ON ( l.type_doc = h.type_doc AND l.trans_no = h.trans_no )
...
def functionPut(con, event):
    ...
    try:
        ...
        sqlUpdate = """
            CALL `in_t_adjustments_edit`(%s, %s, %s, %s, %s, %s, %s)
        """
