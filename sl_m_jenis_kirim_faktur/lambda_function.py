...
def master_table_name():
    return 'sl_01_m_jenis_kirim_faktur'
...
def functionGet(con, event):
    ...
    sql = "SELECT code, description, is_cetak FROM `sl_01_m_jenis_kirim_faktur` WHERE delete_mark = 0 AND active = 1"
    
    if params is not None:
        if params.get('id'):
            sql += " AND code LIKE '%" + params.get('id') + "%'"
...
def functionPost(con, event):
    ...
    sqlInsert = """
    INSERT INTO `sl_01_m_jenis_kirim_faktur`
        (code, description, is_cetak, active, create_by, create_date, delete_mark)
    VALUES
        (%s, %s, %s, %s, %s, %s, 0)
    """

    cur.execute(sqlInsert, (req['code'], req['description'], req['is_cetak'], 
    ...
    sql = "SELECT code, description, is_cetak FROM `sl_01_m_jenis_kirim_faktur` WHERE code = %s"
    cur.execute(sql, (id))
    row = cur.fetchone()
    data = {
        "code": row[0],
...
def functionPut(con, event):
    ...
    id = params['id']
    sqlUpdate = """
    UPDATE `sl_01_m_jenis_kirim_faktur`
    SET
        description = %s,
        is_cetak = %s,
        active = %s,
    ...
    cur.execute(sqlUpdate, (req['description'], req['is_cetak'], req['active'], 
