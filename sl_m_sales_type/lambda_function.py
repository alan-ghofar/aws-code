...
def functionGet(con, event):
	...
	sql = "SELECT id, sales_type FROM `sl_01_m_sales_type` WHERE delete_mark = 0 AND active = 1"
...
def functionPost(con, event):
	...
	sqlInsert = """
	INSERT INTO `sl_01_m_sales_type`
		(sales_type, active, create_by, create_date, delete_mark)
	VALUES
		(%s, %s, %s, %s, 0)
	"""

	cur.execute(sqlInsert, (req['sales_type'], req['active'], vToken['id_user'], datetime.date.today()))
	id = con.insert_id()
	inc_db.addAuditMaster(con, get_id_menu(), inc_def.getActionCreate(), id, '', master_table_name(), vToken['id_user'])
	con.commit()

	sql = "SELECT id, sales_type FROM `sl_01_m_sales_type` WHERE id = %s"
	cur.execute(sql, (id))
	row = cur.fetchone()
	data = {
		"id": row[0],
...
def functionPut(con, event):
	...
	cur.execute(sqlUpdate, (req['sales_type'], req['active'], vToken['id_user'], datetime.date.today(), id))
