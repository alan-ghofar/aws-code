...
def functionGet(con, event):
    ...
    sql = "SELECT id, description FROM `sl_01_m_denied_type` WHERE delete_mark = 0 AND active = 1"
...
def functionPost(con, event):
    ...
    try:
        sqlInsert = """
        INSERT INTO `sl_01_m_denied_type`
    ...
    sql = "SELECT id, description FROM `sl_01_m_denied_type` WHERE id = %s"
    cur.execute(sql, (id))
    row = cur.fetchone()
    data = {
        "id": row[0],
