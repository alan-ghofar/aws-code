...
def functionGet(con, event):
    ...
    sql = "SELECT code, name, decimals FROM `in_01_m_item_units` WHERE `delete_mark`=0 AND `active`=1"
    if params is not None:
        if params.get('id'):
            sql += " AND code = '" + params.get('id') + "'"
...
def functionPost(con, event):
    ...
    sqlInsert = """
    INSERT INTO `in_01_m_item_units` 
        ( code, name, decimals, active, create_by, create_date, delete_mark ) 
    VALUES 
        (%s, %s, %s, %s, %s, %s, 0)
    """
    cur.execute(sqlInsert, (req['code'], req['name'], req['decimals'], req['active'], 
    ...
    sql = "SELECT code, name, decimals FROM `in_01_m_item_units` WHERE code LIKE '%" + req['code'] + "%'"
    cur.execute(sql)
    row = cur.fetchone()
    data = {
        "unit_code": row[0],
...
def functionPut(con, event):
    ...
    WHERE 
       `code` = %s
    """
    cur.execute(sqlUpdate, (req['name'], req['decimals'], 
    ...
    sql = "SELECT code, name, decimals FROM `in_01_m_item_units` WHERE code = %s"
    cur.execute(sql, id)
    row = cur.fetchone()
    data = {
        "unit_code": row[0],
...
def functionDelete(con, event):
    ...
    params = event['queryStringParameters']
    id = params['id']
    sqlUpdate = "UPDATE `in_01_m_item_units` SET delete_by = %s, delete_date = %s, delete_mark = %s WHERE code = %s"
