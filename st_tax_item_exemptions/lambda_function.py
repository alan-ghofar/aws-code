...
def master_table_name():
    return 'st_03_tax_item_exemptions'
...
def functionGet(con, event):
    ...
    sql = """
    SELECT
        tti.tax_type_item_id,
        tti.name tax_type_item_name,
        tt.tax_type_id,
        tt.name tax_type_name
    FROM
        `st_03_tax_item_exemptions` tie
        LEFT JOIN `st_02_tax_type_item` tti ON ( tti.tax_type_item_id = tie.tax_type_item_id ) 
        LEFT JOIN `st_02_tax_types` tt ON ( tt.tax_type_id = tie.tax_type_id ) 
    WHERE
        tie.delete_mark = 0 
        AND tie.active = 1
    """
    if params is not None:
        if params.get('id'):
            sql += " AND tie.tax_type_item_id = " + params.get('id') 
    ...
    for row in myresult:
        data = {
            "tax_type_item_id": row[0],
            "tax_type_item_name": row[1],
            "tax_type_id": row[2],
            "tax_type_name": row[3]
        }
        allData.append(data)
...
def functionPost(con, event):
    ...
    for row in req['tax_type_id']:
        tax_type_id = row['id']
        tax_type_item_id = req['tax_type_item_id']
        ...
        id = tax_type_item_id
        keterangan_audit_master = "id_master = tax_type_item_id; tax_type_id = " + str(tax_type_id)
    ...
    WHERE
        tie.delete_mark = 0 
        AND tie.active = 1
        AND tie.tax_type_item_id = %s
    """
    
    cur.execute(sql, (req['tax_type_item_id']))
    myresult = cur.fetchall()
...
def functionDelete(con, event):
    ...
    id = params['id']
    sqlDelete = """
    UPDATE `st_03_tax_item_exemptions` 
    SET 
        delete_by = %s, 
        delete_date = %s, 
        delete_mark = 1 
    ...
    keterangan_audit_master = 'id_master = tax_type_item_id'
