...
def functionGet(con, event):
    ...
    FROM
        `sc_02_role_item` rt
        LEFT JOIN `sc_01_role` r ON ( r.id = rt.id_role )
        LEFT JOIN `sc_02_menu` m ON ( m.id = rt.id_menu ) 
    WHERE
        rt.delete_mark = 0 
        AND rt.active = 1
    """
...
def functionPost(con, event):
    ...
    sqlInsert = "INSERT INTO `sc_02_role_item` (  id_role, id_menu, create_by, create_date, active, delete_mark) VALUES (%s, %s, %s, %s, 1, 0)"
    cur.execute(sqlInsert, (req['id_role'],  req['id_menu'],
...
def functionPut(con, event):
    ...
    vToken = event['data_user']
    id = params['id']
    sqlUpdate = "UPDATE `sc_02_role_item` SET id_role=%s, id_menu=%s, update_by=%s, update_date=%s, active=%s WHERE id=%s"
    cur.execute(sqlUpdate, (req['id_role'], req['id_menu'], vToken['id_user'],
...
def functionDelete(con, event):
    ...
    vToken = event['data_user']
    id = params['id']
    sqlUpdate = "UPDATE `sc_02_role_item` SET delete_by=%s, delete_date=%s, delete_mark=%s WHERE id=%s"
