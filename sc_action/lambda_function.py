...
def functionGet(con, event):
    ...
    sql = "SELECT * FROM `sc_01_action` WHERE `delete_mark`=0 AND `active`=1"
...
def functionPost(con, event):
    ...
    sqlInsert = "INSERT INTO `sc_01_action` ( nama_action, nilai, create_by, create_date, active, delete_mark ) VALUES (%s, %s, %s, %s, 1, 0)"
    cur.execute(sqlInsert, (req['nama_action'], req['nilai'],
    ...
    sql = "SELECT id, nama_action, nilai FROM `sc_01_action` WHERE id = %s"
    cur.execute(sql, (id))
    row = cur.fetchone()
    data = {
        "id": row[0],
...
def functionPut(con, event):
    ...
    params = event['queryStringParameters']
    id = params['id']
    sqlUpdate = "UPDATE `sc_01_action` SET `nama_action` = %s, `nilai` = %s, `update_by` = %s, `update_date` = %s, `active` = %s WHERE id=%s"
    cur.execute(sqlUpdate, (req['nama_action'], req['nilai'],  vToken['id_user'],
...
def functionDelete(con, event):
    ...
    params = event['queryStringParameters']
    id = params['id']
    sqlUpdate = "UPDATE `sc_01_action` SET delete_by=%s, delete_date=%s, delete_mark=%s WHERE id=%s"
