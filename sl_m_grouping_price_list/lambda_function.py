...
tableName = 'sl_02_m_grouping_price_list'
...
def functionGet(con, event):
    ...
    sql = "SELECT id, description FROM `sl_02_m_grouping_price_list` WHERE delete_mark = 0 AND active = 1"
...
def functionPost(con, event):
    ...
    try:
        sqlInsert = """
        INSERT INTO `sl_02_m_grouping_price_list`
    ...
    sql = "SELECT id, description FROM `sl_02_m_grouping_price_list` WHERE id = %s"
    cur.execute(sql, (id))
    row = cur.fetchone()
    data = {
        "id": row[0],
...
def functionPut(con, event):
    ...
    try:
        sqlUpdate = """
        UPDATE `sl_02_m_grouping_price_list`
        SET
            description = %s,
            active = %s,
            update_by = %s,
        ...
        cur.execute(sqlUpdate, (req['description'], req['active'], vToken['id_user'], datetime.datetime.now(tz_JKT), id))
...
def functionDelete(con, event):
    ...
    cur.execute(sqlDelete, (vToken['id_user'], datetime.datetime.now(tz_JKT), params['id']))
