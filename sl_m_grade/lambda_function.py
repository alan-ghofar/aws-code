...
def functionGet(con, event):
    ...
    sql = "SELECT grade_id, description, amount, active FROM `sl_01_m_grade` WHERE delete_mark = 0 AND active = 1"
    
    if params is not None:
        if params.get('id'):
            sql += " AND grade_id = " + params.get('id')
...
def functionPost(con, event):
    ...
    try:
        sqlInsert = """
        INSERT INTO `sl_01_m_grade`
            (description, amount, active, create_by, create_date, delete_mark)
        VALUES
            (%s, %s, %s, %s, %s, 0)
        """
        
        cur.execute(sqlInsert, (req['description'], req['amount'], 
...
def functionPut(con, event):
    ...
    try:
        ...
        cur.execute(sqlUpdate, (req['description'], req['amount'], req['active'], vToken['id_user'], datetime.datetime.now(tz_JKT), id))
