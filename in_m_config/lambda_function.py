...
def functionGet(con, event):
    ...
    try:
        sql = "SELECT `kode`, `kelompok`, `nilai`, `keterangan` FROM `in_01_m_config` WHERE `active` = 1 AND `delete_mark` = 0"
...
def functionPost(con, event):
    ...
    sqlInsert = """
    INSERT INTO `in_01_m_config` 
        ( `kode`, `kelompok`, `nilai`, `keterangan`, `active`, `create_by`, `create_date`, `delete_mark` ) 
    VALUES 
        (%s, %s, %s, %s, %s, %s, %s, 0)
    """
    cur.execute(sqlInsert, (req['kode'], req['kelompok'], req['nilai'], req['keterangan'], req['active'], vToken['id_user'], datetime.date.today()))
    id = con.insert_id()
    inc_db.addAuditMaster(con, idMenu, inc_def.getActionCreate(), id, '', tableName, vToken['id_user'])
    con.commit()
    

    sql = "SELECT `kode`, `kelompok`, `nilai`, `keterangan` FROM `in_01_m_config` WHERE `kode` = %s"
...
def functionPut(con, event):
    ...
    WHERE 
       `kode` = %s
    """
    cur.execute(sqlUpdate, (req['kelompok'], req['nilai'], req['keterangan'], vToken['id_user'], datetime.date.today(), req['active'], id))
...
def functionDelete(con, event):
    ...
    params = event['queryStringParameters']
    id = params['id']
    sqlUpdate = "UPDATE `in_01_m_config` SET delete_by=%s, delete_date=%s, delete_mark=%s WHERE kode = %s"
