...
tableName = 'sl_01_t_sales_quotation-dispatch'
...
def lambda_handler(event, context):
    ...
    if httpMethod == 'GET':
        ...
        pathActionnya = inc_def.getActionCreate() + "/:id"
    elif httpMethod == 'PUT':
        con = db_con.database_connection_write()
...
def functionPost(con, event):
    # return inc_def.send_response_data("Sek, ojo nganggo iki", 403)
    con.begin()
    ...
    try:
        ...
        for row in req['line_items']:
            ...
            if str(rowItem.get('is_combo')) == "1" :
                ...
                for rowCombo in resCombo:
                    ...
                    lineCombo = str(row['id_line_detail']) + "=-="
        ...
        # sqldebug = """
        # call  `sl_sq_dispatch_to_so`
        # ( 
        # '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', 
        # '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', 
        # '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')
        # """.format(idMenu, inKodePerusahaan, req['trans_no'], typeDocument, typeDocumentTo, str(req['customer_code']), branchCode, 
        ...
        # str(req['routes_id']), str(req['status_code']), is_android_toko, req['is_direct'], str(vToken['id_user']), 0, stritm, strCombo)
        ...
        """
        cur.execute(sqlInsert, (idMenu, inKodePerusahaan, req['trans_no'], typeDocument, typeDocumentTo, str(req['customer_code']), branchCode, 
        req['trans_date'], req['expired_date'], str(req['sales_type_id']), str(req['tax_included']), 
        str(req['payment_terms_id']), str(req['ship_via']), req['deliver_to'], req['delivery_address'], 
        req['contact_phone'], req['customer_ref'], req['memo'], str(req['tax_group_id']), 
        str(req['ov_freight']), str(req['ov_discount']), getKodeLokasi, str(req['salesman_id']), 
...
def functionGet(con, event):
    ...
    try:
        ...
        SELECT
            ...
            sq_line.x_quantity,
            sq_line.qty_sent,
            (sq_line.quantity - sq_line.qty_sent) qty_sisa,
            sq_line.unit_price,
        ...
        FROM
            ...
            LEFT JOIN in_01_m_item_codes icode ON ( icode.item_code = sq_line.item_code )
        ...
        HAVING 
            qty_sisa > 0
        ORDER BY 
            sq.trans_no , sq_line.id
        """
        
        # return inc_def.send_response_data(sql, 200)
        
        cur.execute(sql, (kodePerusahaan, typeDocument, inc_def.getStatusCodeApprove()))
