...
def functionPost(con, event):
    ...
    id = req['kode']
    sqlInsert = """
    INSERT INTO `st_01_sys_prefs` 
    ( `kode`, `nilai`, `keterangan`, `for_user`, `create_by`, `create_date`, `active`, `delete_mark` ) 
    VALUES 
    (%s, %s, %s, %s, %s, %s, %s, 0)
    """
    cur.execute(sqlInsert, (req['kode'], req['nilai'], req['keterangan'],
                req['for_user'], vToken['id_user'], datetime.date.today(), req['active']))
...
def functionDelete(con, event):
    ...
    params = event['queryStringParameters']
    id = params['id']
    sqlUpdate = "UPDATE `st_01_sys_prefs` SET delete_by=%s, delete_date=%s, delete_mark=%s WHERE kode=%s"
