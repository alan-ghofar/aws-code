...
import function_create_so as func_create
import function_delete_so as func_delete
import function_edit_so as func_edit


tableName = 'sl_01_t_sales_order'
...
def lambda_handler(event, context):
    ...
    if user_token is None:
        ...
        if vToken == "false":
            ...
            event['typeDocument'] = typeDocument
...
def functionPost(con, event):
    ...
    response_function = func_create.insert_so(con, event)
...
def functionGet(con, event):
    ...
    try:
        ...
        SELECT
        	...
        	so_line.item_tax_type_id tax_type_item_id,
        	so_line.src_id_line
        FROM
...
def functionDelete(con, event):
    ...
    if str(params.get('is_cancel')) == "1":
        response_function = func_delete.is_cancel(con, event)
        return inc_def.send_response_data(response_function['message'], response_function['code'])
    
    if str(params.get('is_close')) == "1":
        response_function = func_delete.is_close(con, event)
        return inc_def.send_response_data(response_function['message'], response_function['code'])
    
    return inc_def.send_response_data("emoticon senyum", 403)


# =============================== FUNCTION EDIT
def functionPut(con, event):
    # return inc_def.send_response_data(event, 403)
    response_function = func_edit.edit_so(con, event)
    return inc_def.send_response_data(response_function['message'], response_function['code'])