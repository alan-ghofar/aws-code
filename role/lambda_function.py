...
def functionGet(con, event):
    ...
    sql = "SELECT * FROM `sc_01_role` WHERE `delete_mark`=0 AND `active`=1"
...
def functionPost(con, event):
    ...
    sqlInsert = "INSERT INTO `sc_01_role` (  nama_role, keterangan, create_by, create_date, active, delete_mark) VALUES (%s, %s, %s, %s, 1, 0)"
    cur.execute(sqlInsert, (req['nama_role'],  req['keterangan'],
    ...
    sql = "SELECT id, nama_role, keterangan FROM `sc_01_role` WHERE id = %s"
    cur.execute(sql, (id))
    row = cur.fetchone()
    data = {
        "id": row[0],
...
def functionPut(con, event):
    ...
    vToken = event['data_user']
    id = params['id']
    sqlUpdate = "UPDATE `sc_01_role` SET nama_role=%s, keterangan=%s, update_by=%s, update_date=%s, active=%s WHERE id=%s"
    cur.execute(sqlUpdate, (req['nama_role'], req['keterangan'],
    ...
    sql = "SELECT id, nama_role, keterangan, create_by, create_date,update_by, update_date, active FROM `sc_01_role` WHERE id = %s"
...
def functionDelete(con, event):
    ...
    vToken = event['data_user']
    id = params['id']
    sqlUpdate = "UPDATE `sc_01_role` SET delete_by=%s, delete_date=%s, delete_mark=%s WHERE id=%s"
