...
tableName = 'in_01_m_class_of_good'
...
def functionGet(con, event):
    ...
    filterCOG = '%'
    sql = """
    SELECT cog_code, description 
    FROM `in_01_m_class_of_good` 
    WHERE `delete_mark`=0 AND `active`=1
    AND cog_code LIKE %s
    """
    if params is not None:
        if params.get('id'):
            filterCOG = params.get('id')
            #sql += " AND cog_code = '" + params.get('id') + "'"

    cur.execute(sql, (filterCOG))
...
def functionPost(con, event):
    ...
    sqlInsert = """
    INSERT INTO `in_01_m_class_of_good` 
    ...
    sql = "SELECT cog_code, description FROM `in_01_m_class_of_good` WHERE cog_code = %s"
    cur.execute(sql, (id))
    row = cur.fetchone()
    data = {
        "cog_code": row[0],
...
def functionDelete(con, event):
    ...
    params = event['queryStringParameters']
    id = params['id']
    sqlUpdate = "UPDATE `in_01_m_class_of_good` SET delete_by=%s, delete_date=%s, delete_mark=%s WHERE cog_code = %s"
