CREATE DEFINER=`permen`@`%` PROCEDURE `dbmodularerp`.`in_adjustments_cancel`(`in_trans_no` INT,`in_type_doc` VARCHAR(10),`in_status_code` VARCHAR(2),`in_created_by` INT, `in_id_menu` INT, `in_kode_perusahaan` VARCHAR(10), in_keterangan VARCHAR(255))
BEGIN	
