CREATE DEFINER=`permen`@`%` PROCEDURE `dbmodularerp`.`sl_sq_cancel`(`in_trans_no` INT,`in_type_doc` VARCHAR(10),`in_status_code` VARCHAR(2),`in_created_by` INT, `in_id_menu` INT, `in_kode_perusahaan` VARCHAR(10), in_keterangan VARCHAR(255))
BEGIN	
...
-- 	UPDATE HEADER -> DELETE MARK DAN TEMAN-TEMANNYA
-- 	UPDATE `sl_01_t_sales_quotation`
-- 	SET
-- 			delete_mark = 1,
-- 			delete_by = in_created_by,
-- 			delete_date = NOW()
-- 	WHERE
-- 			trans_no = in_trans_no
...
-- 	UPDATE DETAIL	
	UPDATE sl_02_t_sales_quotation_details SET quantity = qty_sent WHERE trans_no = in_trans_no AND type_doc = in_type_doc;
