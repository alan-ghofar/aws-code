...
tableName = 'sl_01_m_payment_terms'
...
def functionGet(con, event):
    ...
    sql = "SELECT terms_indicator, terms, days_before_due, day_in_following_month, payment_type FROM `sl_01_m_payment_terms` WHERE delete_mark = 0 AND active = 1"
    
    if params is not None:
        if params.get('id'):
            sql += " AND terms_indicator LIKE '%" + params.get('id') + "%'"
    ...
    for row in result:
        data = {
            "terms_indicator": row["terms_indicator"],
            "terms": row["terms"],
            "days_before_due": row["days_before_due"],
            "day_in_following_month": row["day_in_following_month"],
...
def functionPost(con, event):
    ...
    try:
        sqlInsert = """
        INSERT INTO `sl_01_m_payment_terms`
            (terms, days_before_due, day_in_following_month, payment_type, active, create_by, create_date, delete_mark)
        VALUES
            (%s, %s, %s, %s, %s, %s, %s, 0)
        """
    
        cur.execute(sqlInsert, (req['terms'], req['days_before_due'], req['day_in_following_month'], req['payment_type'],
...
def functionPut(con, event):
    ...
    try:
        sqlUpdate = """
        UPDATE `sl_01_m_payment_terms`
        SET
            terms = %s,
            days_before_due = %s,
            day_in_following_month = %s,
            payment_type = %s,
            active = %s,
            update_by = %s,
            update_date = %s
        ...
        cur.execute(sqlUpdate, (req['terms'], req['days_before_due'], req['day_in_following_month'], req['payment_type'],
